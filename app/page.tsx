...
async function getStats() {
  try {
    ...
    return { categoriesCount, productsCount, warehousesCount, quotationsCount, companiesCount, error: null };
  } catch (e: any) {
    return { categoriesCount: 0, productsCount: 0, warehousesCount: 0, quotationsCount: 0, companiesCount: 0, error: e.message };
  }
}

export default async function DashboardPage() {
  const { categoriesCount, productsCount, warehousesCount, quotationsCount, companiesCount, error } = await getStats();
  ...
  return (
    <>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        <StatCard title="Active Warehouses" value={warehousesCount} accent="border-l-violet-500" valueColor="text-violet-600" href="/warehouses" icon="🏭" />
        <StatCard title="Categories" value={categoriesCount} accent="border-l-blue-500" valueColor="text-blue-600" href="/categories" icon="📂" />
        <StatCard title="Products" value={productsCount} accent="border-l-emerald-500" valueColor="text-emerald-600" href="/products" icon="📦" />
        <StatCard title="Inventory Items" value={0} accent="border-l-cyan-500" valueColor="text-cyan-600" href="/inventory" icon="📊" />
        <StatCard title="Quotations" value={quotationsCount} accent="border-l-amber-500" valueColor="text-amber-600" href="/quotations/list" icon="📋" />
        <StatCard title="Companies" value={companiesCount} accent="border-l-rose-500" valueColor="text-rose-600" href="/company" icon="🏢" />
      </div>
