// app/api/quotations/[id]/route.ts  — key change: syncInvoice on PUT when SAVED
...
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  const q = await prisma.quotation.findUnique({ where: { id: Number(id) }, include: includeBlock });
...
export async function PUT(req: NextRequest, { params }: Ctx) {
  try {
    ...
    // ── Sync invoice if SAVED ──────────────────────────────────────────────────
    if (updated.status === "SAVED") {
      const company = await prisma.company.findUnique({ where: { id: updated.companyId } });
      if (company) {
        await syncInvoice(qid, company.name, body, items, fixedCosts).catch(console.error);
      }
    }
