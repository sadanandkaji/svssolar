...
type Quotation = {
  ...
  roundedPrice: string; advancePayment: string; balanceDue: string;
  ...
  fixedCosts: Array<{
    ...
    gstRate: string; total: string; included: boolean;
  }>;
};
...
export default function SimpleQuotationPage() {
  ...
  return (
    <>
      ...
      <div className="print:hidden bg-[#1a237e] text-white px-6 py-3 flex items-center justify-between sticky top-0 z-10 shadow">
        <h1 className="text-base font-bold">Simple Quotation Preview</h1>
      ...
      <div className="quotation-root max-w-[900px] mx-auto bg-white my-6 print:my-0 shadow-lg print:shadow-none font-sans text-sm text-slate-800 border border-slate-200 print:border-0">
        ...
        <div className="flex items-start gap-4 p-6 pb-4 border-b-2 border-[#1a237e]">
          ...
          <div className="flex-1">
            ...
            {q.company.ownerName && <p className="text-sm text-slate-600 mt-0.5">Proprietor: {q.company.ownerName}</p>}
        ...
        <div className="flex justify-between items-center px-6 py-3 bg-slate-50 border-b border-slate-200 text-sm">
          ...
          <div><span className="font-bold">Date: </span>{fmtDate(q.quoteDate)}</div>
          <div><span className="font-bold">Valid Until: </span>{expiryDate(q.quoteDate)}</div>
        </div>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-[#1a237e] text-white">
              <th className="px-3 py-2.5 text-center font-semibold text-xs w-12">S.No</th>
              <th className="px-3 py-2.5 text-left font-semibold text-xs">DESCRIPTION</th>
              <th className="px-3 py-2.5 text-center font-semibold text-xs w-20">QTY.</th>
              <th className="px-3 py-2.5 text-right font-semibold text-xs w-32">UNIT PRICE (₹)</th>
              <th className="px-3 py-2.5 text-right font-semibold text-xs w-32">AMOUNT (₹)</th>
            </tr>
          </thead>
          <tbody>
            {allItems.map((it)=>(
              <tr key={it.sno} className="border-b border-slate-100">
                ...
                <td className="px-3 py-2.5 text-right font-semibold font-mono">{fmt(it.total)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {/* Total Section */}
        <div className="grid grid-cols-2 gap-0 border-t-2 border-slate-300">
          {/* Left: Bank Details */}
          ...
          <div className="px-5 py-5">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between py-1 border-b border-slate-100">
                ...
                <span className="font-mono font-medium">₹ {fmt(grandTotal)}</span>
              </div>
              {totalTax > 0 && (
                <div className="flex justify-between py-1 border-b border-slate-100">
              ...
              {Number(q.advancePayment) > 0 && (
                <div className="flex justify-between py-1 border-b border-slate-100">
                  ...
                  <span className="font-mono font-medium">₹ {fmt(q.advancePayment)}</span>
                </div>
              )}
              {Number(q.balanceDue) > 0 && (
                <div className="flex justify-between py-1 border-b border-slate-100">
                  <span className="text-slate-600">Balance Due</span>
                  <span className="font-mono font-medium">₹ {fmt(q.balanceDue)}</span>
                </div>
              )}
            </div>

            {/* Grand Total Box */}
            <div className="mt-3 bg-[#1a237e] text-white rounded px-4 py-3 flex justify-between items-center">
              <span className="font-bold text-base">TOTAL AMOUNT</span>
              <span className="font-bold text-xl font-mono">₹ {fmt(roundedPrice)}</span>
            </div>

            <div className="mt-3">
              <p className="text-xs font-bold text-slate-700">Amount in Words:</p>
