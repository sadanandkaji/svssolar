...
type SessionUser = {
  ...
  companyId: number; companyName: string; isOwner: boolean;
};

type Company = {
  id: number; name: string; ownerName: string | null; address: string | null;
...
type Employee = {
  ...
  role: "OWNER" | "ADMIN" | "STAFF"; isActive: boolean; createdAt: string;
};

type CompanyForm = {
  name: string; ownerName: string; address: string; gstNumber: string;
  contact: string; email: string; logoUrl: string; bankName: string;
  branchName: string; accountName: string; accountNumber: string;
  ifscCode: string; upiId: string; upiQrUrl: string;
};
...
function emptyCompanyForm(c?: Company): CompanyForm {
  return {
    ...
    ifscCode: c?.ifscCode ?? "", upiId: c?.upiId ?? "", upiQrUrl: c?.upiQrUrl ?? "",
  };
}
function emptyEmpForm(): EmpForm   { return { name: "", email: "", password: "", role: "STAFF" }; }
function emptyCredForm(u?: SessionUser): CredForm { return { name: u?.name ?? "", email: u?.email ?? "", currentPassword: "", newPassword: "", confirmPassword: "" }; }
...
function Field({ label, value, onChange, type = "text", required, textarea, readOnly, placeholder }: {
  ...
  return (
    <div>
      ...
      {textarea
        ? <textarea className={cls} rows={3} value={value} readOnly={readOnly} placeholder={placeholder} onChange={readOnly ? undefined : e => onChange?.(e.target.value)} />
        : <input type={type} className={cls} value={value} readOnly={readOnly} placeholder={placeholder} onChange={readOnly ? undefined : e => onChange?.(e.target.value)} />
      }
    </div>
  );
}
...
function RoleBadge({ role }: { role: string }) {
  ...
  return <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${s[role] ?? s.STAFF}`}>{role}</span>;
}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl">
        ...
        <div className="flex gap-3 justify-end">
          <button onClick={onCancel} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
          <button onClick={onConfirm} className={`rounded-lg px-4 py-2 text-sm font-medium text-white ${danger ? "bg-red-600 hover:bg-red-700" : "bg-blue-600 hover:bg-blue-700"}`}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
...
export default function CompanyPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (cRes.ok) {
        ...
        if (list.length > 0) { setCompany(list[0]); setCompanyForm(emptyCompanyForm(list[0])); }
      }
    }
    init();
  }, []);
  ...
  const loadEmployees = useCallback(async () => {
    ...
    } catch { showToast("err", "Failed to load employees"); }
  ...
  async function handleSaveCompany() {
    if (!company || !companyForm.name.trim()) { showToast("err", "Company name is required"); return; }
    ...
    try {
      const res = await fetch(`/api/companies/${company.id}`, {
        ...
        body: JSON.stringify({
          ...
          upiQrUrl:      companyForm.upiQrUrl.trim()      || null,
        }),
      });
      const data = await res.json();
  ...
  async function handleAddEmployee() {
    if (!empForm.name.trim() || !empForm.email.trim() || !empForm.password) {
      showToast("err", "Name, email, and password are required"); return;
    }
    setEmpSaving(true);
    try {
      ...
      setShowAddEmp(false); setEmpForm(emptyEmpForm());
      await loadEmployees();
    } catch (e: any) { showToast("err", e.message); }
  ...
  async function handleUpdateEmployee(id: number) {
    ...
    try {
      ...
      const res  = await fetch(`/api/employees/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to update");
      showToast("ok", "Employee updated");
      setEditEmpId(null); setEditEmpData({});
      await loadEmployees();
    } catch (e: any) { showToast("err", e.message); }
    finally { setEmpSaving(false); }
  }
  ...
  async function handleToggleActive(emp: Employee) {
    try {
      const res  = await fetch(`/api/employees/${emp.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: !emp.isActive }) });
      const data = await res.json();
  ...
  async function handleDeleteEmployee(id: number) {
    try {
      ...
      setEmpDeleteId(null);
      await loadEmployees();
    } catch (e: any) { showToast("err", e.message); setEmpDeleteId(null); }
  }

  async function handleSaveCreds(e: React.FormEvent) {
    e.preventDefault();
    const payload: Record<string, any> = {};
    if (credForm.name.trim() && credForm.name.trim() !== currentUser?.name) payload.name = credForm.name.trim();
    if (credForm.email.trim() && credForm.email.trim() !== currentUser?.email) payload.email = credForm.email.trim();
    if (credForm.newPassword) {
      if (!credForm.currentPassword) { showToast("err", "Enter your current password to set a new one"); return; }
      if (credForm.newPassword !== credForm.confirmPassword) { showToast("err", "New passwords do not match"); return; }
      if (credForm.newPassword.length < 6) { showToast("err", "Password must be at least 6 characters"); return; }
      payload.currentPassword = credForm.currentPassword;
      payload.newPassword     = credForm.newPassword;
    }
    if (Object.keys(payload).length === 0) { showToast("err", "No changes to save"); return; }
    setSavingCred(true);
    try {
      const res  = await fetch("/api/auth/update-credentials", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to update");
      ...
      if (payload.email || payload.newPassword) {
        ...
        const u = { ...currentUser!, name: data.name };
        setCurrentUser(u); setCredForm(emptyCredForm(u));
      }
    } catch (e: any) { showToast("err", e.message); }
  ...
  const sc = (k: keyof CompanyForm) => (v: string) => setCompanyForm(p => ({ ...p, [k]: v }));
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-b border-slate-200 px-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-slate-800 pt-4">{company?.name ?? "Company"}</h2>
              {company?.ownerName && <p className="text-xs text-slate-500 pb-1">Proprietor: {company.ownerName}</p>}
            </div>
          ...
          <div className="flex gap-0 -mb-px mt-1">
            {([
              ...
              <button key={t.id} onClick={() => { setTab(t.id); setEditingCompany(false); }}
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Field label="Company / Trade Name" required value={cf.name} onChange={sc("name")} readOnly={!editingCompany} placeholder="e.g. Sri Veerabhadraeshwara Solar" />
                <Field label="Owner / Legal Name" value={cf.ownerName} onChange={sc("ownerName")} readOnly={!editingCompany} placeholder="e.g. DARSHAN K M" />
                <div className="md:col-span-2">
                  <Field label="Address" value={cf.address} onChange={sc("address")} readOnly={!editingCompany} textarea placeholder="Building No., Street, City, District, State, PIN" />
                </div>
                <Field label="GST Number (GSTIN)" value={cf.gstNumber} onChange={v => setCompanyForm(p => ({ ...p, gstNumber: v.toUpperCase() }))} readOnly={!editingCompany} placeholder="e.g. 29EKVPD6110H1ZX" />
                <Field label="Contact Number" value={cf.contact} onChange={sc("contact")} readOnly={!editingCompany} placeholder="e.g. +919945117650" />
                <Field label="Email" type="email" value={cf.email} onChange={sc("email")} readOnly={!editingCompany} placeholder="e.g. svssolar@gmail.com" />
                <Field label="Logo URL" value={cf.logoUrl} onChange={sc("logoUrl")} readOnly={!editingCompany} placeholder="https://... (optional)" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Field label="Bank Name" value={cf.bankName} onChange={sc("bankName")} readOnly={!editingCompany} placeholder="e.g. Canara Bank" />
                <Field label="Branch" value={cf.branchName} onChange={sc("branchName")} readOnly={!editingCompany} placeholder="e.g. Chickjajur" />
                <Field label="Account Name" value={cf.accountName} onChange={v => setCompanyForm(p => ({ ...p, accountName: v.toUpperCase() }))} readOnly={!editingCompany} placeholder="e.g. SRI VEERABHADRESHWARA SWAMY SOLAR" />
                <Field label="Account Number" value={cf.accountNumber} onChange={sc("accountNumber")} readOnly={!editingCompany} placeholder="e.g. 120035340906" />
                <Field label="IFSC Code" value={cf.ifscCode} onChange={v => setCompanyForm(p => ({ ...p, ifscCode: v.toUpperCase() }))} readOnly={!editingCompany} placeholder="e.g. CNRB0000452" />
                <Field label="UPI ID" value={cf.upiId} onChange={sc("upiId")} readOnly={!editingCompany} placeholder="e.g. svssolar@okaxis" />
              </div>
            </div>
            ...
            <div>
              <h3 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500 mb-4 pb-2 border-b border-slate-100">
                <span className="text-violet-500 text-base">📲</span> UPI QR Code
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                <div>
                  <Field
                    ...
                    value={cf.upiQrUrl}
                    onChange={sc("upiQrUrl")}
                    readOnly={!editingCompany}
                    placeholder="https://... (paste direct image URL of your UPI QR code)"
                  />
                  <p className="text-xs text-slate-400 mt-1.5">
                    When payment type is <strong>UPI</strong>, this QR image will appear on the invoice for customers to scan and pay.
                  </p>
                </div>
                {cf.upiQrUrl && (
                  <div className="flex flex-col items-start gap-1">
                    ...
                    />
                    {cf.upiId && <p className="text-xs font-mono text-violet-700 mt-1">{cf.upiId}</p>}
                  </div>
                )}
              </div>
            </div>
              ...
              <div>
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                  <div>
                    ...
                    {company.gstNumber && <p className="mt-2 font-mono text-xs text-slate-500">GSTIN: <span className="font-semibold text-slate-700">{company.gstNumber}</span></p>}
                  ...
                  <div className="space-y-3">
                    ...
                    {company.upiQrUrl && (
                      <div className="border-l-4 border-violet-400 pl-4 bg-white rounded-r-xl py-3 pr-4">
                        <p className="font-bold text-violet-700 text-xs uppercase tracking-wide mb-2">UPI QR Code</p>
                        <img src={company.upiQrUrl} alt="UPI QR" className="h-20 w-20 object-contain rounded border border-slate-200 bg-white p-1" />
                      </div>
                    )}
                    {company.logoUrl && (
            ...
            {editingCompany && (
              <div className="flex items-center gap-3 pt-2 border-t border-slate-100">
                <button onClick={handleSaveCompany} disabled={savingCompany}
                  ...
                  {savingCompany ? <><div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" />Saving...</> : "💾 Save Changes"}
                </button>
                <button onClick={() => { setEditingCompany(false); setCompanyForm(emptyCompanyForm(company ?? undefined)); }}
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              {currentUser?.isOwner && !showAddEmp && (
                <button onClick={() => { setShowAddEmp(true); setEmpForm(emptyEmpForm()); }}
            ...
            {showAddEmp && currentUser?.isOwner && (
              <div className="mb-5 rounded-xl border border-blue-200 bg-blue-50/60 p-5">
                <h4 className="text-sm font-bold text-blue-800 mb-4 flex items-center gap-2"><span>➕</span> New Employee</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1 uppercase tracking-wide">Full Name *</label>
                    <input value={empForm.name} onChange={e => setEmpForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Ravi Kumar"
                  ...
                  <div>
                    ...
                    <input type="email" value={empForm.email} onChange={e => setEmpForm(p => ({ ...p, email: e.target.value }))} placeholder="ravi@company.com"
                      className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1 uppercase tracking-wide">Password *</label>
                    <input type="password" value={empForm.password} onChange={e => setEmpForm(p => ({ ...p, password: e.target.value }))} placeholder="Min. 6 characters"
                      className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                  </div>
                  <div>
            ...
            {empLoading ? (
              <div className="flex items-center justify-center py-12 text-slate-400">
                <div className="h-6 w-6 animate-spin rounded-full border-4 border-slate-200 border-t-slate-500 mr-2" />Loading employees...
              </div>
            ) : employees.length === 0 ? (
              <div className="rounded-xl border-2 border-dashed border-slate-200 py-12 text-center text-slate-400">
                ...
                {currentUser?.isOwner && <button onClick={() => setShowAddEmp(true)} className="mt-2 text-blue-600 text-sm underline">Add your first employee</button>}
              </div>
            ) : (
              <div className="space-y-2">
                {employees.map(emp => (
                  <div key={emp.id} className={`rounded-xl border p-4 transition-all ${emp.isActive ? "border-slate-200 bg-white" : "border-slate-100 bg-slate-50"}`}>
                    {editEmpId === emp.id ? (
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                          <div>
                            ...
                            <input value={editEmpData.name ?? emp.name} onChange={e => setEditEmpData(p => ({ ...p, name: e.target.value }))}
                          ...
                          <div>
                            ...
                            <input type="email" value={editEmpData.email ?? emp.email} onChange={e => setEditEmpData(p => ({ ...p, email: e.target.value }))}
                              className="w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-slate-500 mb-1">New Password</label>
                            <input type="password" value={editEmpData.password ?? ""} onChange={e => setEditEmpData(p => ({ ...p, password: e.target.value }))}
                              placeholder="Leave blank to keep"
                              className="w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-slate-500 mb-1">Role</label>
                            <select value={editEmpData.role ?? emp.role} onChange={e => setEditEmpData(p => ({ ...p, role: e.target.value as "ADMIN" | "STAFF" }))}
                              className="w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                        ...
                        <div className="flex gap-2 mt-3">
                          ...
                          <button onClick={() => { setEditEmpId(null); setEditEmpData({}); }}
                            className="rounded-lg border border-slate-300 bg-white px-4 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <p className={`text-sm font-semibold truncate ${emp.isActive ? "text-slate-800" : "text-slate-400 line-through"}`}>{emp.name}</p>
                              <RoleBadge role={emp.role} />
                              {!emp.isActive && <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-medium text-slate-500">Inactive</span>}
                              {emp.id === currentUser?.id && <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-medium text-emerald-700">You</span>}
                            </div>
                        ...
                        {currentUser?.isOwner && emp.role !== "OWNER" && (
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button onClick={() => { setEditEmpId(emp.id); setEditEmpData({ name: emp.name, email: emp.email, role: emp.role }); }}
                              className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition">✏️ Edit</button>
                            <button onClick={() => handleToggleActive(emp)}
                              className={`rounded-lg border px-2.5 py-1.5 text-xs font-medium transition ${emp.isActive ? "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" : "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100"}`}>
                              {emp.isActive ? "Deactivate" : "Activate"}
                            </button>
                            <button onClick={() => setEmpDeleteId(emp.id)}
                              className="rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-100 transition">🗑 Remove</button>
                          </div>
                        )}
                      </div>
                    )}
            ...
            {!currentUser?.isOwner && <p className="mt-5 text-center text-xs text-slate-400">Only the owner can add or manage team members.</p>}
          </div>
        )}
        ...
        {tab === "account" && (
          <div className="p-6">
            <div className="max-w-lg">
              ...
              <p className="text-xs text-slate-500 mb-6">Update your display name, login email, or password. If you change your email or password, you will be signed out and need to log in again.</p>
              <form onSubmit={handleSaveCreds} className="space-y-5">
                <div>
                  ...
                  <input value={credForm.name} onChange={e => setCredForm(p => ({ ...p, name: e.target.value }))} placeholder="Your full name"
                    className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 mb-1.5">Login Email</label>
                  <input type="email" value={credForm.email} onChange={e => setCredForm(p => ({ ...p, email: e.target.value }))} placeholder="your@email.com"
                    className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" />
                </div>
                <div className="border-t border-slate-100 pt-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input type="password" value={credForm.currentPassword} onChange={e => setCredForm(p => ({ ...p, currentPassword: e.target.value }))}
                    ...
                    <div>
                      ...
                      <input type="password" value={credForm.newPassword} onChange={e => setCredForm(p => ({ ...p, newPassword: e.target.value }))}
                    ...
                    <div>
                      ...
                      <input type="password" value={credForm.confirmPassword} onChange={e => setCredForm(p => ({ ...p, confirmPassword: e.target.value }))}
                ...
                <div className="pt-2 flex items-center gap-3">
                  <button type="submit" disabled={savingCred}
                    className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700 disabled:opacity-60">
                    {savingCred ? <><div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" />Saving...</> : "💾 Save Changes"}
                  </button>
                  <button type="button" onClick={() => setCredForm(emptyCredForm(currentUser ?? undefined))}
                    className="rounded-lg border border-slate-300 bg-white px-6 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Reset</button>
                </div>
