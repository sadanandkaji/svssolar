...
// app/(app)/invoices/[id]/edit/EditInvoiceClient.tsx
import { useEffect, useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";

type Company   = { id: number; name: string; address: string | null; gstNumber: string | null; contact: string | null; email: string | null; logoUrl: string | null; bankName: string | null; branchName: string | null; accountName: string | null; accountNumber: string | null; ifscCode: string | null };
...
export default function EditInvoiceClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function loadAll() {
      try {
        ...
        const [cosRes, catsRes, prodsRes, invRes] = await Promise.all([
          fetch("/api/companies"),
        ...
        if (!invRes.ok) { setError("Invoice not found"); setReady(true); return; }

        const [cos, cats, prodsData, inv] = await Promise.all([
          cosRes.json(), catsRes.json(), prodsRes.json(), invRes.json(),
        ]);

        setCompanies(Array.isArray(cos) ? cos : []);
        setCategories(Array.isArray(cats) ? cats : []);
        setAllProducts(prodsData.products || []);

        // ── Populate every form field from the invoice ──
        ...
        setSystemSizeKw(inv.systemSizeKw != null ? String(inv.systemSizeKw) : "");
        setPanelType(inv.panelType ?? "");
        setPanelWattage(inv.panelWattage != null ? String(inv.panelWattage) : "");
        setPanelCount(inv.panelCount != null ? String(inv.panelCount) : "");
        setPhase(inv.phase ?? "");
        setDiscountPercent(String(inv.discountPercent ?? "0"));
        setAdvancePayment(inv.advancePayment != null && Number(inv.advancePayment) > 0 ? String(inv.advancePayment) : "");
        ...
        setFixedCosts((inv.fixedCosts ?? []).map((fc: any) => ({
          id:       uid(),
          label:    fc.label    ?? "",
          cost:     String(fc.cost    ?? "0"),
          rateNote: fc.rateNote ?? "",
          gstRate:  String(fc.gstRate ?? "18"),
  ...
  // Keep selectedCompany in sync when companyId changes manually
  useEffect(() => {
  ...
  function updateFC(fcId: string, patch: Partial<FixedCost>) {
    setFixedCosts(prev => prev.map(fc => {
      if (fc.id !== fcId) return fc;
      const next = { ...fc, ...patch };
  ...
  async function handleSave() {
    ...
    try {
      ...
      showToast("ok","Invoice updated!");
      setTimeout(()=>router.push(`/invoices/${id}/view`), 600);
    } catch (e: any) {
  ...
  return (
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 -mt-6 bg-gray-100 min-h-screen">
      ...
      <div className="bg-violet-800 text-white px-6 py-3 flex items-center justify-between shadow">
        <h1 className="text-lg font-bold">🧾 Edit Invoice — <span className="font-mono">{invoiceNumber}</span></h1>
        <div className="flex gap-2">
          <Link href={`/invoices/${id}/view`} className="text-sm bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded transition">
            👁 View Invoice
          </Link>
          <Link href="/invoices" className="text-sm bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded transition">
            ← All Invoices
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
            <div className="border-b border-slate-200 px-6 py-3"><h2 className="text-base font-bold text-violet-800">🏢 Company</h2></div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input className={`${inp} bg-slate-50 font-mono text-violet-700`} readOnly value={invoiceNumber}/>
              </div>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className={lbl}>System Type</label>
              <select className={inp} value={systemType} onChange={e=>setSystemType(e.target.value)}>
                <option value="">-- Select --</option>{SYSTEM_TYPES.map(s=><option key={s}>{s}</option>)}
              </select></div>
            ...
            <div><label className={lbl}>Panel Type</label>
              <select className={inp} value={panelType} onChange={e=>setPanelType(e.target.value)}>
                <option value="">-- Select --</option>{PANEL_TYPES.map(p=><option key={p}>{p}</option>)}
              </select></div>
            ...
            <div><label className={lbl}>Phase</label>
              <select className={inp} value={phase} onChange={e=>setPhase(e.target.value)}>
                <option value="">-- Select --</option>{PHASES.map(p=><option key={p}>{p}</option>)}
              </select></div>
        ...
        {/* Products */}
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
          <div className="border-b border-slate-200 px-6 py-3"><h2 className="text-base font-bold text-violet-800">📦 Products</h2></div>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2">
                        <select className="w-32 border border-slate-300 rounded px-2 py-1.5 text-sm" value={item.categoryName}
                      ...
                      <td className="px-3 py-2">
                        <select className="w-40 border border-slate-300 rounded px-2 py-1.5 text-sm" value={item.productName}
                          onChange={e=>onSelectProduct(item.id,e.target.value)} disabled={!item.categoryName}>
                          <option value="">{item.categoryName?"Select":"Category first"}</option>
                          {catProducts.map(p=><option key={p.id} value={p.name}>{p.name}</option>)}
                        </select>
                      </td>
                      <td className="px-3 py-2"><input className="w-24 border border-slate-300 rounded px-2 py-1.5 text-sm font-mono" value={item.hsnCode} onChange={e=>updateItem(item.id,{hsnCode:e.target.value})} placeholder="HSN"/></td>
                      <td className="px-3 py-2"><textarea className="w-32 border border-slate-300 rounded px-2 py-1.5 text-sm resize-none" rows={1} value={item.description} onChange={e=>updateItem(item.id,{description:e.target.value})}/></td>
                      <td className="px-3 py-2"><input type="number" className="w-28 border border-slate-300 rounded px-2 py-1.5 text-sm" value={item.unitPrice} onChange={e=>updateItem(item.id,{unitPrice:e.target.value})}/></td>
                      <td className="px-3 py-2"><input type="number" className="w-16 border border-slate-300 rounded px-2 py-1.5 text-sm" value={item.quantity} onChange={e=>updateItem(item.id,{quantity:e.target.value})}/></td>
                      <td className="px-3 py-2">
                        <select className="w-20 border border-slate-300 rounded px-2 py-1.5 text-sm" value={item.gstRate} onChange={e=>updateItem(item.id,{gstRate:e.target.value})}>
                          {GST_OPTIONS.map(g=><option key={g} value={g}>{g}%</option>)}
                        </select>
                      </td>
                      <td className="px-3 py-2"><input className="w-28 border border-slate-200 bg-slate-100 rounded px-2 py-1.5 text-sm" readOnly value={fmtINR(item.totalPrice)}/></td>
              ...
              <tbody className="divide-y divide-slate-100">
                {fixedCosts.map((fc,idx)=>(
                  <tr key={fc.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2"><input type="number" className="w-32 border border-slate-300 rounded px-2 py-1.5 text-sm" value={fc.cost} onChange={e=>updateFC(fc.id,{cost:e.target.value})}/></td>
                    <td className="px-3 py-2"><input className="w-32 border border-slate-200 bg-slate-50 rounded px-2 py-1.5 text-sm" value={fc.rateNote} onChange={e=>updateFC(fc.id,{rateNote:e.target.value})}/></td>
                    <td className="px-3 py-2">
                      <select className="w-20 border border-slate-300 rounded px-2 py-1.5 text-sm" value={fc.gstRate} onChange={e=>updateFC(fc.id,{gstRate:e.target.value})}>
                        {GST_OPTIONS.map(g=><option key={g} value={g}>{g}%</option>)}
                      </select>
                    </td>
                    <td className="px-3 py-2"><input className="w-28 border border-slate-200 bg-slate-100 rounded px-2 py-1.5 text-sm" readOnly value={fmtINR(fc.total)}/></td>
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-3">
              {[["Subtotal:", fmtINR(subtotal)],["Total GST:", fmtINR(totalGst)]].map(([l,v])=>(
                <div key={l} className="flex items-center gap-4">
                  <span className="w-40 text-sm font-medium text-slate-700 shrink-0">{l}</span>
                  <input readOnly className="flex-1 border border-slate-300 rounded px-3 py-2 text-sm bg-slate-100" value={v}/>
                </div>
              ))}
              <div className="flex items-center gap-4">
                <label className="w-40 text-sm font-medium text-slate-700 shrink-0">Discount %:</label>
              ...
              <div className="flex items-center gap-4">
                <span className="w-40 text-sm font-medium text-slate-700 shrink-0">Discount Amount:</span>
                <input readOnly className="flex-1 border border-slate-300 rounded px-3 py-2 text-sm bg-slate-100" value={fmtINR(discountAmt)}/>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-4">
                <span className="w-40 text-sm font-semibold text-violet-700 shrink-0">Final Price:</span>
                <input readOnly className="flex-1 border border-slate-300 rounded px-3 py-2 text-sm bg-slate-100 font-semibold text-violet-700" value={fmtINR(finalPrice)}/>
              </div>
              <div className="flex items-center gap-4">
                <span className="w-40 text-sm font-medium text-slate-700 shrink-0">Rounded Price:</span>
                <input readOnly className="flex-1 border border-slate-300 rounded px-3 py-2 text-sm bg-slate-100" value={fmtINR(roundedPrice)}/>
              </div>
              <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-4">
                <span className="w-40 text-sm font-semibold text-violet-700 shrink-0">Balance Due:</span>
                <input readOnly className="flex-1 border border-slate-300 rounded px-3 py-2 text-sm bg-slate-100 font-semibold text-violet-700" value={fmtINR(balanceDue)}/>
              </div>
              <div className="flex items-center gap-4">
        ...
        <div className="flex items-center justify-center gap-4 pb-8">
          ...
          <Link href={`/invoices/${id}/view`}
            className="bg-slate-500 hover:bg-slate-600 text-white px-8 py-2.5 rounded font-medium text-sm">
            ↺ Cancel
          </Link>
        </div>
